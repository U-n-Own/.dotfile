...
function createCommands(ctx, kpmController) {
    ...
    cmds.push(vscode_1.commands.registerCommand('codetime.connectSlack', () => {
        (0, SlackManager_1.connectSlackWorkspace)();
    }));
    cmds.push(vscode_1.commands.registerCommand('codetime.disconnectSlackWorkspace', (auth_id) => {
        if (auth_id) {
            (0, SlackManager_1.disconnectSlackAuth)(auth_id);
        }
        else {
            (0, SlackManager_1.disconnectSlackWorkspace)();
        }
    }));
    // INTEGRATION DISCONECT
    cmds.push(vscode_1.commands.registerCommand('codetime.disconnectIntegration', (payload) => {
        (0, HttpClient_1.appDelete)(`/data_sources/integration_connections/${payload.id}`).then((resp) => {
            (0, ProgressManager_1.progressIt)('Disconnecting integration...', DataController_1.diconnectIntegration, [payload.id]);
        });
    }));
