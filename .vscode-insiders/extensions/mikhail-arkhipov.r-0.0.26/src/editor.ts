...
export function getFilePath(fileUri?: Uri): string {
    let filePath = '';
        ...
        if (activeEditor !== undefined && activeEditor !== null) {
            if (!activeEditor.document.isUntitled) {
                if (activeEditor.document.languageId === RLanguage.language) {
                    ...
                    window.showErrorMessage('The active file is not a R source file.');
                }
            } else {
                window.showErrorMessage('The active file needs to be saved before it can be sourced.');
            }
        } else {
            window.showErrorMessage('No open R file to source.');
        }
    } else {
